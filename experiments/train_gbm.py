...
from models.tree_models import GBMMultiOutput
...
def run():
    ...
    model = GBMMultiOutput(**config.GBM_PARAMS)
    ...
    print(f"\n[GBM] Test RMSE={rmse:.5f}  R2={r2:.5f}")
    for i, (r, r2h) in enumerate(ph, start=1):
        print(f"  horizon {i}: RMSE={r:.5f}  R2={r2h:.5f}")

    plot_test_suite(preds_u, trues_u, ph, model_prefix="gbm")

    joblib.dump(model.model, config.CHECKPOINT_GBM)
    print("Saved:", config.CHECKPOINT_GBM)

    return {"name": "GBM", "rmse": rmse, "r2": r2, "per_horizon": ph}
